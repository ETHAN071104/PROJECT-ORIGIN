...
export function Portrait({ type, mentor, active = false, visionUpgraded = false, learningUpgraded = false }: PortraitProps) {
  return (
    <div className={`portrait portrait-${type} ${mentor ? `portrait-${mentor}` : ''} ${active ? 'is-speaking' : ''}`}>
      <div className="portrait-frame">
        {type === 'player' && <div className={`portrait-bot ${visionUpgraded ? 'has-vision-upgrade' : ''} ${learningUpgraded ? 'has-learning-upgrade' : ''}`}><i /><i /><b /></div>}
